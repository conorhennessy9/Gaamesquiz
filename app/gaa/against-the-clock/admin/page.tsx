...
import {
  getGAAClockQuestions,
  createGAAClockQuestion,
  updateGAAClockQuestion,
  deleteGAAClockQuestion,
  type GAAClockQuestion,
} from "../actions"

export default function GAAClockAdmin() {
  const [isAuthenticated, setIsAuthenticated] = useState(false)
  const [password, setPassword] = useState("")
  const [questions, setQuestions] = useState<GAAClockQuestion[]>([])
  const [editingQuestion, setEditingQuestion] = useState<GAAClockQuestion | null>(null)
  ...
  const loadQuestions = async () => {
    setIsLoading(true)
    try {
      const loadedQuestions = await getGAAClockQuestions()
  ...
  const createQuestion = async () => {
    ...
    setIsSaving(true)
    try {
      const result = await createGAAClockQuestion({
  ...
  const updateQuestion = async () => {
    ...
    setIsSaving(true)
    try {
      const result = await updateGAAClockQuestion(editingQuestion.id, {
  ...
  const deleteQuestion = async (id: number) => {
    ...
    setIsSaving(true)
    try {
      const result = await deleteGAAClockQuestion(id)
      ...
      <header className="p-4">
        <div className="container mx-auto">
          <div className="flex items-center justify-between">
            ...
            <h1 className="text-white text-xl font-bold">GAA Clock Admin</h1>
            <div></div>
          </div>
        </div>
      </header>
                ...
                <div className="space-y-4 pt-4 border-t">
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      placeholder="e.g., Kerry, Dublin, Galway, Mayo, Cork"
                      rows={3}
                    />
                  </div>
