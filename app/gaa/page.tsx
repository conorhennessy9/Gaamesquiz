...
const games = [
  {
    ...
    desc: "A question is set — you have to name all the correct answers before you rack up 3 strikes. How well do you really know your GAA?",
    stat: "10 answers",
  },
  {
    id: "against-the-clock",
    href: "/gaa/against-the-clock",
      ...
      <main className="container mx-auto px-4 py-12 max-w-4xl">
        ...
        <div className="mb-12">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-xs font-bold tracking-widest uppercase text-white/40">GAA</span>
          </div>
          ...
          <p className="text-base text-white/40 max-w-lg leading-relaxed">
            Daily challenges for GAA fans. New questions every day — test yourself on players, clubs, counties and history.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-12">
          {games.map((game) => (
            <Link key={game.id} href={game.href}>
              <div className="group relative rounded-xl border border-white/[0.08] bg-[#111] p-6 hover:border-emerald-500/30 hover:bg-[#161616] transition-all duration-200 h-full flex flex-col cursor-pointer">
                <div className="flex items-center justify-between mb-5">
                  <span className="rounded-md bg-emerald-500/10 px-2.5 py-1 text-xs font-bold text-emerald-300 border border-emerald-500/20">
                    {game.stat}
                  </span>
                </div>

                <h2 className="text-xl font-black text-white mb-1">{game.name}</h2>
                <p className="text-xs font-semibold tracking-wide text-emerald-400 mb-3 uppercase">{game.tagline}</p>
